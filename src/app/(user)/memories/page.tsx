...
import { ArrowLeft, Camera, Plus, X, Heart, Save } from "lucide-react";
...
const Page = () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (formData.title && formData.date) {
            ...
            setFormData({ title: '', date: '', note: '', photos: [] });
            setShowModal(false);
        }
    };
    ...
    const handleEmojiToggle = (emoji: string) => {
        setFormData(prev => ({
            ...prev,
            photos: prev.photos.includes(emoji)
                ? prev.photos.filter(p => p !== emoji)
                : [...prev.photos, emoji]
        }));
    };
            ...
            <div className="p-4">
                <div className="bg-white rounded-2xl p-4 shadow-lg mb-4">
                    ...
                    <input
                        ...
                        className="w-full p-2 border-2 border-blue-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-400"
                    />
                    {filterDate && (
                        <button
                    ...
                    <div className="mt-4">
                        <h4 className="font-semibold text-gray-700 mb-2">
                            Dates with memories are marked in{" "}
                ...
                <div className="space-y-4">
                    {filteredEvents.length > 0 ? (
                        filteredEvents.map((event) => (
                            <div
                                key={event.id}
                                className="bg-white rounded-2xl p-4 shadow-lg cursor-pointer transform hover:scale-[1.03] transition-transform duration-300 border-l-4 border-blue-400"
                            >
                                <div className="flex items-start space-x-4">
                                    <div className="bg-gradient-to-br from-blue-100 to-pink-100 p-3 rounded-full">
                ...
                <button
                    ...
                    className="fixed bottom-6 right-6 bg-gradient-to-r from-blue-400 to-pink-400 text-white p-4 rounded-full shadow-lg hover:shadow-xl transform hover:scale-110 transition-all duration-200 z-40"
                >
